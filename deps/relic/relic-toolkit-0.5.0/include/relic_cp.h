 ...
 */
#if ALLOC == AUTO
typedef relic_rlc_rsa_st rlc_rsa_t[1];
#else
typedef relic_rlc_rsa_st *rlc_rsa_t;
#endif
...
#if ALLOC == DYNAMIC
#define rsa_new(A)															\
	A = (rlc_rsa_t)calloc(1, sizeof(relic_rlc_rsa_st));									\
	if (A == NULL) {														\
		THROW(ERR_NO_MEMORY);												\
	}																		\
	bn_null((A)->e);														\
...
#elif ALLOC == STACK
#define rsa_new(A)															\
	A = (rlc_rsa_t)alloca(sizeof(relic_rlc_rsa_st));										\
	bn_new((A)->e);															\
	bn_new((A)->n);															\
	bn_new((A)->d);															\
	bn_new((A)->dp);														\
